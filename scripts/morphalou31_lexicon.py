...
from utils.Utils import create_logger #, SEPAR1, SEPAR2
...
def add_flection(toks,txt2pos,txtpos2lem,lempos2txt,txtlempos2feats,base_lem,base_pos,base_spos,base_genre):
    ...
    fdo.write('\t'.join([txt, lem, '|'.join([pos, mode, temps, pers, nombre, genre])])+'\n')
#    for pho in phos:
...
if __name__ == '__main__':
    ...
    with open(fin+'.lexicon.lex', 'w') as fdo:
        with open(fin,'r') as fdi:
            for l in fdi:
                ...
                ### add entry
                if add_flection(toks[9:17],txt2pos,txtpos2lem,lempos2txt,txtlempos2feats,lem_str,pos_str,spos_str,genre_str):
                    m += 1
                    
    logging.info('Loaded Lexicon with {} entries, output {} entries'.format(n,m))
    logging.info('Saved {}'.format(fin+'.lexicon.lex'))
    
    with open(fin+'.lexicon.pickle', 'wb') as fdo:
        pickle.dump([txtpos2lem, txt2pos, lempos2txt, txtlempos2feats], fdo, protocol=-1)

    logging.info('Saved {}'.format(fin+'.lexicon.pickle'))
